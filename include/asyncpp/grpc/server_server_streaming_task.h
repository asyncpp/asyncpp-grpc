...
namespace asyncpp::grpc {
	...
	struct server_server_streaming_task {
		...
		server_server_streaming_task(handle_t h) : m_coro{h} {
			if (!m_coro) throw std::invalid_argument("m_coro is invalid");
			m_coro.resume();
		}

		/// \brief Move constructor
		server_server_streaming_task(server_server_streaming_task&& t) noexcept : m_coro(std::exchange(t.m_coro, {})) {}

		/// \brief Move assignment
		server_server_streaming_task& operator=(server_server_streaming_task&& t) noexcept {
			m_coro = std::exchange(t.m_coro, m_coro);
			return *this;
		}

		server_server_streaming_task(const server_server_streaming_task& o) : m_coro{o.m_coro} {
			if (m_coro) m_coro.promise().ref();
		}

		server_server_streaming_task& operator=(const server_server_streaming_task& o) {
